...
int __fastcall commandlist::functions::sub_140654FD8(__int64 a1, int* a2, const char* a3, int a4) {
	if (commandlist::combos == 1)
		if (a4 == 0)
			return osub_140654FD8(a1, a2, a3, a4);
		else
			return 0;
	return osub_140654FD8(a1, a2, a3, a4);
}
...
__int64 __fastcall commandlist::functions::sub_140654848(__int64 a1) {
	...
	sub_7FF6BFE58250 osub_7FF6BFE58250 = (sub_7FF6BFE58250)(plugin::moduleBase + 0x657650 + 0xC00);
	typedef signed __int64(__fastcall* ccGame_GetPlayerLeader) (unsigned int a1);
	ccGame_GetPlayerLeader occGame_GetPlayerLeader = (ccGame_GetPlayerLeader)(plugin::moduleBase + 0x85A948 + 0xC00);
	commandlist::customCLRun = 0;
	typedef signed __int64(__fastcall* ccGetCharCode) (int a1);
	ccGetCharCode occGetCharCode = (ccGetCharCode)(plugin::moduleBase + 0x5289FC + 0xC00);
	...
	if (clchar.isvalid()) {
		...
		*(DWORD*)(PlayerLeader + 0xC94) = 0;
		if (*(DWORD*)(PlayerLeader + 0xCBC) == 1)
			...
			clchar.awake = 0;
		const std::array<unsigned char, 5> combocommandlist = util::memory::Modify::read_bytes<5>(plugin::moduleBase + 0x654155 + 0xC00);
		const std::array<unsigned char, 5> jutsucommandlist = util::memory::Modify::read_bytes<5>(plugin::moduleBase + 0x654182 + 0xC00);
		const std::array<unsigned char, 5> combocommandlist2 = util::memory::Modify::read_bytes<5>(plugin::moduleBase + 0x654194 + 0xC00);
		util::memory::Modify::write_bytes<5>(plugin::moduleBase + 0x654155 + 0xC00, { 0x90, 0x90, 0x90, 0x90, 0x90 });
		util::memory::Modify::write_bytes<5>(plugin::moduleBase + 0x654182 + 0xC00, { 0x90, 0x90, 0x90, 0x90, 0x90 });
		util::memory::Modify::write_bytes<5>(plugin::moduleBase + 0x654194 + 0xC00, { 0x90, 0x90, 0x90, 0x90, 0x90 });
		...
		auto retval = osub_140654848(a1);
		util::memory::Modify::write_bytes<5>(plugin::moduleBase + 0x654155 + 0xC00, { combocommandlist[0], combocommandlist[1], combocommandlist[2], combocommandlist[3], combocommandlist[4] });
...
__int64* __fastcall commandlist::functions::strcpy(void* Dst, char* Src, size_t Size) {
	...
	if (commandlist::customCLRun == 1 && src == "Kaguya Otsutsuki") {
		commandlist::customCLRun = 0;
		return ostrcpy(Dst, &clchar.name[0], clchar.name.size());
	}
	if (commandlist::customCLRun == 1 && src == "Kaguya Otsutsuki (Awakening)") {
		commandlist::customCLRun = 0;
		return ostrcpy(Dst, &clchar.awaname[0], clchar.awaname.size());
	}
	return ostrcpy(Dst, Src, Size);
}
...
__int64 __fastcall commandlist::functions::ccUiGetMessage(__int64 MessageToDecode) {
	...
	__int64 retval = occUiGetMessage(MessageToDecode);
	return retval;
}
